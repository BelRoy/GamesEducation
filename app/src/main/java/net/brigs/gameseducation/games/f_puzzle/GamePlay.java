...
public class GamePlay extends Activity implements Constants, OnClickListener {
	...
	class TileArray extends ArrayList<TextView> implements OnTouchListener {
		...
		public TileArray(int nRows, int nColumns, int nSteps, String prevSaves) {
			...
			TextView empty = new TextView(GamePlay.this);
		...
		public void updateDisplay() {
			for (int i = 0; i < size(); i++) {
				if (i != emptyPos) {
					LayoutParams tvLParams = (LayoutParams) get(
							i).getLayoutParams();
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			LayoutParams vPar = (LayoutParams) v.getLayoutParams();
